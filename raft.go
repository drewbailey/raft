...
func (r *Raft) runLeader(ch <-chan RPC) {
	...
	// is closed as soon as we step down. Ussed to stop
	// extra goroutines.
...
// and vote for ourself. This has the side affect of incrementing
// the current term. The response channel returned is used to wait
// for all the responses (include a vote for ourself).
...
func (r *Raft) replicate(triggerCh, stopCh chan struct{}, peer net.Addr) {
	// Initialize a timer to fire immediately since
...
func (r *Raft) heartbeat(peer net.Addr) {
	...
	if err := r.trans.AppendEntries(peer, &req, &resp); err != nil {
		log.Printf("[ERR] Failed to hearbeat with %v: %v", peer, err)
	}
}
